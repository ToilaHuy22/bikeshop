...
prevSlide = () => {
	hero_slide_index =
		hero_slide_index - 1 < 0 ? hero_slide_items.length - 1 : hero_slide_index - 1;
	showSlide(hero_slide_index);
};
...
//pause/play when mouse in/out slider
hero_slide.addEventListener('mouseover', () => (hero_slide_play = true));

hero_slide.addEventListener('mouseleave', () => (hero_slide_play = false));
...
// auto play
setInterval(() => {
	if (!hero_slide_play) return nextSlide();
}, 8000);
...
isElInViewPort = (el) => {
	...
	return (
		rect.top <=
		(window.innerHeight - distance || document.documentElement.clientHeight - distance)
	);
};
